...
void nRF24_Init(NRF24_HandleTypeDef *dev, SPI_HandleTypeDef *spiHandle) {
	// Write to registers their initial values

	dev->spiHandle = spiHandle;

	nRF24_WriteReg(dev, nRF24_REG_CONFIG, 0x08);
	nRF24_WriteReg(dev, nRF24_REG_EN_AA, 0x3F);
	nRF24_WriteReg(dev, nRF24_REG_EN_RXADDR, 0x03);
	nRF24_WriteReg(dev, nRF24_REG_SETUP_AW, 0x03);
	nRF24_WriteReg(dev, nRF24_REG_SETUP_RETR, 0x03);
	nRF24_WriteReg(dev, nRF24_REG_RF_CH, 0x02);
	nRF24_WriteReg(dev, nRF24_REG_RF_SETUP, 0x0E);
	nRF24_WriteReg(dev, nRF24_REG_STATUS, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_RX_PW_P0, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_RX_PW_P1, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_RX_PW_P2, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_RX_PW_P3, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_RX_PW_P4, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_RX_PW_P5, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_DYNPD, 0x00);
	nRF24_WriteReg(dev, nRF24_REG_FEATURE, 0x00);
...
uint8_t nRF24_Check(NRF24_HandleTypeDef *dev) {
	uint8_t rxbuf[5];
	uint8_t i;
	uint8_t *ptr = (uint8_t *)nRF24_TEST_ADDR;

	// Write test TX address and read TX_ADDR register
	nRF24_WriteMBReg(dev, nRF24_CMD_W_REGISTER | nRF24_REG_TX_ADDR, ptr, 5);
	nRF24_ReadMBReg(dev, nRF24_CMD_R_REGISTER | nRF24_REG_TX_ADDR, rxbuf, 5);
...
void nRF24_AssignCEPin(NRF24_HandleTypeDef *dev, GPIO_TypeDef *CE_Port, uint16_t CE_Pin){
	dev->CE_Port = CE_Port;
	dev->CE_Pin = CE_Pin;
}

void nRF24_AssignCSNPin(NRF24_HandleTypeDef *dev, GPIO_TypeDef *CSN_Port, uint16_t CSN_Pin){
	dev->CSN_Port = CSN_Port;
	dev->CSN_Pin = CSN_Pin;
}
void nRF24_AssignLEDPin(NRF24_HandleTypeDef *dev, GPIO_TypeDef *LED_Port, uint16_t LED_Pin){
...
void nRF24_SetPowerMode(NRF24_HandleTypeDef *dev, uint8_t mode) {
	uint8_t reg;
...
void nRF24_SetOperationalMode(NRF24_HandleTypeDef *dev, uint8_t mode) {
	uint8_t reg;
...
void nRF24_SetDynamicPayloadLength(NRF24_HandleTypeDef *dev, uint8_t mode) {
	uint8_t reg;
...
// input:
//   mode - status, 1 or 0
void nRF24_SetPayloadWithAck(NRF24_HandleTypeDef *dev, uint8_t mode) {
	uint8_t reg;
	reg  = nRF24_ReadReg(dev, nRF24_REG_FEATURE);
	if(mode) {
...
void nRF24_SetCRCScheme(NRF24_HandleTypeDef *dev, uint8_t scheme) {
	uint8_t reg;
...
void nRF24_SetRFChannel(NRF24_HandleTypeDef *dev, uint8_t channel) {
...
void nRF24_SetAutoRetr(NRF24_HandleTypeDef *dev, uint8_t ard, uint8_t arc) {
	// Set auto retransmit settings (SETUP_RETR register)
	nRF24_WriteReg(dev, nRF24_REG_SETUP_RETR, (uint8_t)((ard << 4) | (arc & nRF24_MASK_RETR_ARC)));
}
...
void nRF24_SetAddrWidth(NRF24_HandleTypeDef *dev, uint8_t addr_width) {
...
void nRF24_SetAddr(NRF24_HandleTypeDef *dev, uint8_t pipe, const uint8_t *addr) {
	uint8_t addr_width;
	...
	switch (pipe) {
		...
		case nRF24_PIPE1:
			...
			addr += addr_width;
			nRF24_CSN_L(dev);
			nRF24_LL_RW(dev, nRF24_CMD_W_REGISTER | nRF24_ADDR_REGS[pipe]);
			do {
				nRF24_LL_RW(dev, *addr--);
			} while (addr_width--);
			nRF24_CSN_H(dev);
			break;
		case nRF24_PIPE2:
...
void nRF24_SetTXPower(NRF24_HandleTypeDef *dev, uint8_t tx_pwr) {
	uint8_t reg;
...
void nRF24_SetDataRate(NRF24_HandleTypeDef *dev, uint8_t data_rate) {
	uint8_t reg;
...
void nRF24_SetRXPipe(NRF24_HandleTypeDef *dev, uint8_t pipe, uint8_t aa_state, uint8_t payload_len) {
	uint8_t reg;

	// Enable the specified pipe (EN_RXADDR register)
	reg = (nRF24_ReadReg(dev, nRF24_REG_EN_RXADDR) | (1 << pipe)) & nRF24_MASK_EN_RX;
...
void nRF24_ClosePipe(NRF24_HandleTypeDef *dev, uint8_t pipe) {
	uint8_t reg;
...
// input:
//   pipe - number of the RX pipe, value from 0 to 5
void nRF24_EnableAA(NRF24_HandleTypeDef *dev, uint8_t pipe) {
	uint8_t reg;
...
void nRF24_DisableAA(NRF24_HandleTypeDef *dev, uint8_t pipe) {
	uint8_t reg;
...
uint8_t nRF24_GetStatus(NRF24_HandleTypeDef *dev) {
...
uint8_t nRF24_GetIRQFlags(NRF24_HandleTypeDef *dev) {
	return (nRF24_ReadReg(dev, nRF24_REG_STATUS) & nRF24_MASK_STATUS_IRQ);
}

// Get status of the RX FIFO
// return: one of the nRF24_STATUS_RXFIFO_xx values
uint8_t nRF24_GetStatus_RXFIFO(NRF24_HandleTypeDef *dev) {
...
uint8_t nRF24_GetStatus_TXFIFO(NRF24_HandleTypeDef *dev) {
	return ((nRF24_ReadReg(dev, nRF24_REG_FIFO_STATUS) & nRF24_MASK_TXFIFO) >> 4);
}

// Get pipe number for the payload available for reading from RX FIFO
// return: pipe number or 0x07 if the RX FIFO is empty
uint8_t nRF24_GetRXSource(NRF24_HandleTypeDef *dev) {
...
// Flush the TX FIFO
void nRF24_FlushTX(NRF24_HandleTypeDef *dev) {
	nRF24_WriteReg(dev, nRF24_CMD_FLUSH_TX, nRF24_CMD_NOP);
}

// Flush the RX FIFO
void nRF24_FlushRX(NRF24_HandleTypeDef *dev) {
	nRF24_WriteReg(dev, nRF24_CMD_FLUSH_RX, nRF24_CMD_NOP);
}

// Clear any pending IRQ flags
void nRF24_ClearIRQFlags(NRF24_HandleTypeDef *dev) {
	uint8_t reg;

	// Clear RX_DR, TX_DS and MAX_RT bits of the STATUS register
	reg  = nRF24_ReadReg(dev, nRF24_REG_STATUS);
...
void nRF24_WritePayload(NRF24_HandleTypeDef *dev, uint8_t *pBuf, uint8_t length) {
	nRF24_WriteMBReg(dev, nRF24_CMD_W_TX_PAYLOAD, pBuf, length);
}

static uint8_t nRF24_GetRxDplPayloadWidth(NRF24_HandleTypeDef *dev) {
	uint8_t value;

	nRF24_CSN_L(dev);
	nRF24_LL_RW(dev, nRF24_CMD_R_RX_PL_WID);
	value = nRF24_LL_RW(dev, nRF24_CMD_NOP);
	nRF24_CSN_H(dev);
...
static nRF24_RXResult nRF24_ReadPayloadGeneric(NRF24_HandleTypeDef *dev, uint8_t *pBuf, uint8_t *length, uint8_t dpl) {
	uint8_t pipe;
...
nRF24_RXResult nRF24_ReadPayload(NRF24_HandleTypeDef *dev, uint8_t *pBuf, uint8_t *length) {
	return nRF24_ReadPayloadGeneric(dev, pBuf, length,0);
}

nRF24_RXResult nRF24_ReadPayloadDpl(NRF24_HandleTypeDef *dev, uint8_t *pBuf, uint8_t *length) {
	return nRF24_ReadPayloadGeneric(dev, pBuf, length,1);
}

uint8_t nRF24_GetFeatures(NRF24_HandleTypeDef *dev) {
    return nRF24_ReadReg(dev, nRF24_REG_FEATURE);
}
void nRF24_ActivateFeatures(NRF24_HandleTypeDef *dev) {
    nRF24_CSN_L(dev);
    nRF24_LL_RW(dev, nRF24_CMD_ACTIVATE);
    nRF24_LL_RW(dev, 0x73);
    nRF24_CSN_H(dev);
}
void nRF24_WriteAckPayload(NRF24_HandleTypeDef *dev, nRF24_RXResult pipe, char *payload, uint8_t length) {
	nRF24_CSN_L(dev);
	nRF24_LL_RW(dev, nRF24_CMD_W_ACK_PAYLOAD | pipe);
	while (length--) {
		nRF24_LL_RW(dev, (uint8_t) *payload++);
	}
	nRF24_CSN_H(dev);
...
void nRF24_RX_OFF(NRF24_HandleTypeDef *dev) {
    HAL_GPIO_WritePin(dev->CE_Port, dev->CE_Pin, GPIO_PIN_RESET);
}

void nRF24_RX_ON(NRF24_HandleTypeDef *dev) {
	HAL_GPIO_WritePin(dev->CE_Port, dev->CE_Pin, GPIO_PIN_SET);
}

void nRF24_CSN_L(NRF24_HandleTypeDef *dev) {
    HAL_GPIO_WritePin(dev->CSN_Port, dev->CSN_Pin, GPIO_PIN_RESET);
}

void nRF24_CSN_H(NRF24_HandleTypeDef *dev) {
    HAL_GPIO_WritePin(dev->CSN_Port, dev->CSN_Pin, GPIO_PIN_SET);
}

void Toggle_LED(NRF24_HandleTypeDef *dev) {
	HAL_GPIO_TogglePin(dev->LED_Port, dev->LED_Pin);
}



uint8_t nRF24_LL_RW(NRF24_HandleTypeDef *dev, uint8_t data) {
    // Wait until TX buffer is empty
    uint8_t result;
    if(HAL_SPI_TransmitReceive(dev->spiHandle,&data,&result,1,2000)!=HAL_OK) {
        Error_Handler();
    };
    return result;
}
...
uint8_t nRF24_ReadReg(NRF24_HandleTypeDef *dev, uint8_t reg) {
	uint8_t value;

	nRF24_CSN_L(dev);
	nRF24_LL_RW(dev, reg & nRF24_MASK_REG_MAP);
	value = nRF24_LL_RW(dev, nRF24_CMD_NOP);
	nRF24_CSN_H(dev);
...
void nRF24_WriteReg(NRF24_HandleTypeDef *dev, uint8_t reg, uint8_t value) {
	nRF24_CSN_L(dev);
	if (reg < nRF24_CMD_W_REGISTER) {
		// This is a register access
		nRF24_LL_RW(dev, nRF24_CMD_W_REGISTER | (reg & nRF24_MASK_REG_MAP));
		nRF24_LL_RW(dev, value);
	} else {
...
void nRF24_ReadMBReg(NRF24_HandleTypeDef *dev, uint8_t reg, uint8_t *pBuf, uint8_t count) {
	nRF24_CSN_L(dev);
	nRF24_LL_RW(dev, reg);
	while (count--) {
		*pBuf++ = nRF24_LL_RW(dev, nRF24_CMD_NOP);
	}
	nRF24_CSN_H(dev);
}
...
void nRF24_WriteMBReg(NRF24_HandleTypeDef *dev, uint8_t reg, uint8_t *pBuf, uint8_t count) {
	nRF24_CSN_L(dev);
	nRF24_LL_RW(dev, reg);
	while (count--) {
		nRF24_LL_RW(dev, *pBuf++);
	}
	nRF24_CSN_H(dev);
}
